...
function getField($loader, $fieldname = null)
{
    ...
    } elseif (preg_match('/(?P<name>\w+)\[(?P<index>\w+)\]\[(?P<index2>\w+)\]/', $fieldname, $match)) {
        $name = $match['name'];
        $index2 = $match['index2'];
        $index = $match['index'];

        return $loader->$name[$index][$index2];
    } elseif (preg_match('/(?P<name>\w+)\[(?P<index>\w+)\]/', $fieldname, $match)) {
        $name = $match['name'];
        $index = $match['index'];

        return $loader->$name[$index];
    } elseif (preg_match('/(?P<name>\w+)->(?P<subfield>\w+)/', $fieldname, $match)) {
        $name = $match['name'];
        $subfield = $match['subfield'];

        return $loader->$name->$subfield;
    } elseif (preg_match('/\[(?P<index>\w+)\]\[(?P<index2>\w+)\]/', $fieldname, $match)) {
        $index2 = $match['index2'];
        $index = $match['index'];

        return $loader[$index][$index2];
    } elseif (preg_match('/\[(?P<index>\w+)\]/', $fieldname, $match)) {
        $index = $match['index'];
 ...
 * @tests /tests/Unit/Tests/TestHelpersTest.php
 */
function callProtectedMethod(&$object, $methodName, array $parameters = [])
{
